...
impl BTreeProps {
    ...
    fn is_max_out<T: Ord + Copy>(&self, node: &Node<T>) -> bool {
    ...
    fn insert_non_full<T: Ord + Copy + Default>(&mut self, node: &mut Node<T>, key: T) {
        ...
        if node.is_leaf() {
            ...
            if self.is_max_out(&node.children[u_index]) {
...
impl BTree<T> where T: Ord + Copy + Default + Debug {
    ...
    pub fn insert(&mut self, key: T) {
        if self.props.is_max_out(&self.root) {

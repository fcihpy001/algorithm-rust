...
impl<T> LinkedList<T> {
    ...
    pub fn insert_at_ith(&mut self, index: u32, data: T) {
        ...
        if let Some(mut head_node) = self.head {
            for _ in 0..index {
                unsafe {
                    match (*head_node.as_ptr()).next {
                        None => panic("index out of bounds"),
                        Some(next_ptr) => head_node = next_ptr
                    }
                }
            }
            ...
            let mut node = Box::new(Node::new(data));
            unsafe {
                node.prev = (*head_node.as_ptr()).prev;
                node.next = Some(head_node);

                if let Some(p) = (*head_node.as_ptr()).prev {
                    let node_ptr = Some(
                    ...
                    (*head_node.as_ptr()).prev = node_ptr;
                    self.length += 1;
                }
            }
        }
